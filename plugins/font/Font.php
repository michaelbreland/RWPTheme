...
class FontPlugin {
    ...
    function __construct() {
        ...
        //jquery font plugin
        wp_register_script('font-plugin', plugins_url('/js/jquery.fontPlugin.js?pver=' . $this->version, __FILE__), 'jquery');
        //jquery ui file
        wp_register_script('colorpicker2', plugins_url('/js/colorpicker.js', __FILE__), 'jquery');
        wp_register_script('pluginscripts', plugins_url('/js/pluginscripts.js?pver=' . $this->version, __FILE__), 'jquery');
        /*
          AJAX HANDLERS
         */
	...
	*/
    ...
    function set_font_settings($settings) {
		...
		for ($i = 0; $i < count($setObj->presets); $i++) {
            ...
            //generate css for this preset
            $styles .= $this->getRules($preset);
        }
    ...
    //get font face for the font of given id from fontsforweb.com
    function getRules($preset) {
        //$jsonPreset = json_encode($preset);
        $presetUrl = urlencode(serialize($preset));
        $blogUrl = get_bloginfo('wpurl');
        $gotFontFace = $this->file_get_contents2($this->baseUrl . '/api/getfontface?blogurl=' . $blogUrl . '&preset=' . $presetUrl);
        return $gotFontFace;
    }
    //add task to a list
    ...
    function loadPostFonts($content) {
        ...
        foreach ($content as $post) {
            ...
            $fontsIds = array_unique($searchResults[1]);
            $fontsNames = array_unique($searchResults[2]);
    ...
    function loadTinymceStyles() {
        ...
        if (!function_exists('tdav_css')) {
            function tdav_css($wp) {
                $wp .= ',' . trailingslashit(plugin_dir_url(__FILE__)) . 'generateEditorCSS.php'; //?' . implode('&' . $fontPairsUrlArr);
                return $wp;
            }
        }
    ...
    function addScripts() {
        ...
        wp_enqueue_script('jquery-jcarousel', false, 'jquery');
        //wp_enqueue_script( 'jquery-ui');
        wp_enqueue_script('jquery-ui-draggable');
        wp_enqueue_script('jquery-ui-slider');
        wp_enqueue_script('colorpicker2');
